...
class templ_anything_slider extends WP_Widget {
	...
	// prints the widget
		...
		$readmore = empty($instance['readmore']) ? 'Leia Mais' : apply_filters('widget_readmore', $instance['readmore']);
 		 ?>
 ...
 <script type="text/javascript" src="<?php bloginfo('template_directory'); ?>/js/jquery-1.3.2.min.js"></script>
		...
		<script type="text/javascript" src="<?php bloginfo('template_directory'); ?>/js/jquery.coda-slider-2.0.js"></script>
		 <script type="text/javascript">
            ...
            <?php if($height != '') { ?>   <style type="text/css"> .coda-slider { height:<?php echo $height; ?>px !important; }</style> <?php }?>
	...
	<?php
 	...
 	<?php } } ?>   
                ...
                <?php if($readmore){?>  <p class="clearfix"><a href="<?php the_permalink(); ?>" class="more"><?php echo $readmore; ?></a></p> <?php }?>
              ...
              <?php if($title){?> <h2><?php echo $title; ?></h2> <?php }?>
                ...
                <?php 
					...
					setup_postdata($post);
					 ...
					 <small>										   
						 <?php if(get_option('ptthemes_listing_date')=='Sim' || get_option('ptthemes_listing_date')==''){?>                  
						 <?php _e('Posted on','templatic');?> <span class="post-date"><?php the_time(templ_get_date_format()) ?></span> 
						 <?php } ?>					 
						 <?php if(get_option('ptthemes_listing_comment')=='Sim' || get_option('ptthemes_listing_comment')==''){?>  						  
...
class templ_services_listing extends WP_Widget {
		...
		// prints the widget
			...
			$more1 = empty($instance['more1']) ? 'Leia Mais' : apply_filters('widget_more1', $instance['more1']);
			$more2 = empty($instance['more2']) ? 'Leia Mais' : apply_filters('widget_more2', $instance['more2']);
			$more3 = empty($instance['more3']) ? 'Leia Mais' : apply_filters('widget_more3', $instance['more3']);
			$more4 = empty($instance['more4']) ? 'Leia Mais' : apply_filters('widget_more4', $instance['more4']);
			 ?>			
