...
if($_POST)
{
	if($_POST['your-email'])
	{
		...
		(get_option('blogname') != "") ? $toEmailName = get_option('blogname') : $toEmailName = "Meu Site";
		$toEmail = get_site_emailId();
		
		$subject = $_POST['your-subject'];
		$message = '';
		$message .= '<p>Ol&aacute; '.$toEmailName.',</p>';
		$message .= '<p>Nome : '.$_POST['your-name'].',</p>';
		$message .= '<p>Email : '.$_POST['your-email'].',</p>';
		$message .= '<p>Mensagem : '.nl2br($_POST['your-message']).'</p>';
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset=UTF-8' . "\r\n";
		// Additional headers
		$headers .= 'To: '.$toEmailName.' <'.$toEmail.'>' . "\r\n";
		//$headers .= 'From: '.$_POST['your-name'].' <'.$_POST['your-email'].'>' . "\r\n";
		$headers .= 'From: '.$_POST['your-name'].' <'.get_option('mail_from').'>' . "\r\n";
...
<?php
			...
			?>
<p class="success_msg">
  <?php _e('Thank you, your Information send successfully.','templatic');?>
</p>
<?php
			}
			?>
