      ...
      window.send_to_editor = function(html) {
        if (formfield) {
          ...
          if (itemurl.match(image)) {
            btnContent = '<img src="'+itemurl+'" alt="" /><a href="" class="remove">Remover imagem</a>';
          } else {
            btnContent = '<div class="no_image">'+html+'<a href="" class="remove">Remover</a></div>';
          }
  ...
  inlineEditOption = {
    init: function () {
      ...
      $('.reset', '#the-theme-options').live("click", function () {
        var agree = confirm("Você tem absoluta certeza que quer deletar todas as suas opções salvas?");
        if (agree) {
      ...
      $('#upload-xml').submit(function() {
        var agree = confirm("Você tem certeza que quer importar essas novas configurações?");
        if (agree) {
          return true;
        }
        return false;
      ...
      $('.delete-inline').live("click", function (event) {
        if ($("a.delete-inline").hasClass('disable')) {
          ...
          var agree = confirm("Você tem certeza que deseja deletar esta opção?");
          if (agree) {
    ...
    save_options: function (e) {
	 ...
	 var ajaxurl = ajax_option_url_var+'?ajaxact=save_options';
	  $.post(ajaxurl, d, function (r) {
		if (r != -1) {
          $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>As opções do site foram salvas</div>');
        } else {
          $('.ajax-message').ajaxMessage('<div class="message warning"><span>&nbsp;</span>As opções do site não puderam ser salvas</div>');
        }
      });
      return false;
    },
    reset_options: function () {
      ...
      $.post(ajaxurl, d, function (r) {
        if (r != -1) {
          ...
          $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>As opções do site foram removidas</div>');
        } else {
          $('.ajax-message').ajaxMessage('<div class="message warning"><span>&nbsp;</span>As opções do site não puderam ser removidas</div>');
        }
      });
      return false;
    },
    import_data: function (e) {
      ...
      $.post(ajaxurl, d, function (r) {
        if (r != -1) {
          ...
          $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>Suas opções do site foram importadas com sucesso.</div>');
        } else {
          $('.ajax-message').ajaxMessage('<div class="message warning"><span>&nbsp;</span>Suas opções do site não puderam ser importadas.</div>');
        }
      });
      return false;
    },
    remove: function (b) {
      ...
      $.post(ajaxurl, d, function (r) {
        if (r) {
          if (r == 'removed') {
            $("#option-" + c).remove();
            $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>Opção removida.</div>');
          } else {
      ...
      $('.select').live('change', function () {
        ...
        if (temp_select == 'Heading') {
          ...
          if (temp_select == 'Textarea') {
            $('.regular').hide();
            $('.alternative').show().html('<strong>Contagem de linhas:</strong> Digite um valor númerico para o número de linhas em seu campo de texto.');
            $('.option-desc', '#edit-'+c).show();
            $('.option-options', '#edit-'+c).show();
          } else if (
            ...
            $('.regular').hide();
            $('.alternative').show().html('<strong>Tipo de Post:</strong> Insira seu post_type (tipo de post) personalizado.');
            $('.option-desc', '#edit-'+c).show();
            $('.option-options', '#edit-'+c).show();
          } else {
            $('.option-desc', '#edit-'+c).show();
      ...
      $.post(ajaxurl, d, function (r) {
        if (r) {
          if (r == 'updated') {
            ...
            $("#edit-" + e).remove();
            $("#option-" + e).show();
            $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>Opção adicionada.</div>');
            $('#framework-settings').tableDnD({
              onDragClass: "dragging",
              onDrop: function(table, row) {
                d = {
    ...
    edit: function (b) {
      ...
      if ("#option-" + c == '#option-1') {
        ...
        $('.description', editRow).html('O primeiro item precisa ser um cabeçalho.');
      }
      ...
      $.post(ajaxurl, d, function (r) {
        if (r) {
          if (r == 'updated') {
            ...
            $("#edit-" + e).remove();
            $("#option-" + e).show();
            $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>Opção salva.</div>');
          } else {
            $('.ajax-message').ajaxMessage('<div class="message warning"><span>&nbsp;</span>'+r+'</div>');
          }
        } else {
